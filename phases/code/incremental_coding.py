...
To accomplish the user's task, we completed some sprints through a executable software with multiple files \
implemented via {language} and get the source code below:
...
by inheriting existing source code and writing one or multiple files and make sure that every detail of the architecture is, \
in the end, implemented as code. {gui}
...
Please note that the code MUST be fully functional. Ensure to implement all functions. No placeholders (such as 'pass' in Python).
"""
...
class IncrementalCoding(BasePhaseRepositoryImpl):
    def __init__(
        self,
        model_config,
